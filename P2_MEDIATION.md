...
Two independent routes produce e:
...
**Convergence witness:** Route 1 uses the P3 operation (extract [0,0]) on the P2 generator (h). Route 2 uses the P1 operation (determinant = composition invariant) on the P1 generator (R) through P2's mechanism (exp). Two projection faces, one constant. Third instance of the convergence pattern (after SUBSTRATE Thm 0.3r' for ρ-regulation, CATEGORY Thm 1.10a for Dist uniqueness). Forced by CATEGORY Thm 4.3 — every Dist morphism carries all three readings simultaneously.
