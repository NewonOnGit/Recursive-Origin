...
The projections are f'' = f's first observational act. π₁ reads one basis solution's index and discards the other. The kernel IS the blindness — the structural content π₁ cannot access. The sector sweep (SUBSTRATE §8½), which evaluates exp(X(s)) at the [0,0] entry and discards the other three entries, is the continuous descendant of this discrete projection. What π₁ does to pairs, the [0,0] extraction does to matrices.
...
Given equivalence relations from §2, what maps preserve them? Three arguments converge on the same answer, confirmed by five-way elimination.
